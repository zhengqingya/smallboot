...
@Slf4j
@Component
@ConditionalOnProperty(value = "netty.websocket.enable", havingValue = "true", matchIfMissing = false)
...
//                        pipeline.addLast("handler-header", new NettyHeaderHandler());
        ...
        try {
            ...
            // 就绪标志
            this.isReady = true;
            log.info("【netty】websocket server 初始化完成，端口：{}", this.port);
            // 等待服务端口关闭
            //channel.closeFuture().sync();
        } catch (Exception e) {
            log.info("【netty】websocket server 初始化异常：", e);
        }
    }
    ...
    public void stop() {
        ...
        this.isReady = false;
        log.info("【netty】websocket server 停止");
    }
