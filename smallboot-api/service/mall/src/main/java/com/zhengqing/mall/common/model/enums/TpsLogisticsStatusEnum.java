...
@Getter
@AllArgsConstructor
public enum TpsLogisticsStatusEnum {
    ...
    private static final List<TpsLogisticsStatusEnum> LIST = Lists.newArrayList();

    static {
        LIST.addAll(Arrays.asList(TpsLogisticsStatusEnum.values()));
    }

    /**
     * 根据物流状态查找相应枚举
     */
    public static TpsLogisticsStatusEnum getEnum(Integer status) {
        for (TpsLogisticsStatusEnum itemEnum : LIST) {
            if (itemEnum.getStatus().equals(status)) {
                return itemEnum;
            }
        }
        return TpsLogisticsStatusEnum.DEFAULT;
    }
