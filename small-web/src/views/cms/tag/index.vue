<template>
  <base-wrapper>
    ...
    <base-table-p ref="baseTableRef" api="cms_job_tag.page" :params="listQuery">
    ...
    <base-dialog v-model="dialogVisible" :title="dialogTitleObj[dialogStatus]" width="40%">
      <el-form ref="dataFormRef" :model="form" :rules="rules" label-width="50px">
...
async function handleDelete(row) {
  let res = await proxy.$api.cms_job_tag.delete({ id: row.id });
  refreshTableData();
...
function submitForm() {
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      let res = await proxy.$api.cms_job_tag[form.id ? 'update' : 'add'](form);
      proxy.submitOk(res.message);
      refreshTableData();
      dialogVisible = false;
    }
