  ...
  <view class="h-full m-30 bg-color-white">
    ...
    <view class="">
      ...
      <view class="item">
        ...
        <picker class="value" mode="date" :value="userObj.birthday" @change="changeBirthday">
      ...
      <view class="item">
        ...
        <input class="value" value="If i were you." maxlength="20" placeholder="请输入签名" />
      </view>
      <!-- <view class="item">
...
async function chooseavatar({ detail }) {
  ...
  // 将临时访问地址 转存 服务器
  let res = await proxy.$api.common.upload(detail.avatarUrl);
  userObj.value.avatarUrl = res.data.url;
}
